...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased bg-gray-100">
        <div class="max-w-4xl mx-auto py-6">
            <h1 class="text-xl font-bold">Adicionar elemento ao evento de {{ $event->client->name }}</h1>
                ...
                <h2>Itens</h2>
                    <ul>
                        @foreach ($items as $item)
                            <li>
                                <form action="{{route('event.store_item_to_checklist', ['event_id'=>$event->id])}}" method="post">
                                    @csrf
